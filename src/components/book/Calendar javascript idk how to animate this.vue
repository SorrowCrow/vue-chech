...
var insertMenu = function () {
    ...
    let row = document.getElementsByClassName(`${menuId.toString()}row`);

    insertAfter(choose[0], row[0]);
    setTimeout(function () {
        choose[0].classList.remove("hidden");
    }, 250);
};
...
var insertSelect = function (txt) {
    ...
    let row = document.getElementById(menuId);
    let opened = document.getElementsByClassName("opened");
    row.appendChild(opened[0]);
    opened.css({
        "grid-row": `${menuId + 1}/${menuId + 7}`,
    });
    // console.log(txt);
    document.getElementsByClassName("pa").text(txt);
    opened.removeClass("hidden", 250);
};

var removeSelect = function () {
    let opened = document.getElementsByClassName("opened");
    opened[0].classList.add("hidden", 250);
};

var removeMenu = function () {
    let choose = document.getElementsByClassName("choose");
    choose[0].classList.add("hidden", 250);
};
...
window.onload = function () {
    let openedInner = document.getElementsByClassName("opened__inner-select");
    openedInner[0].addEventListener("click", function () {
        removeSelect();
        insertMenu();
    });

    var hoursButtons = document.getElementsByClassName("hoursButton");
    hoursButtons.forEach((button) => {
        button.addEventListener("click", function () {
            // console.log(button.text());
            removeMenu();
            insertSelect(button.text());
        });
    });

    var daysDates = document.getElementsByClassName("days-dates");
    let length = daysDates.length;
    for (var i = 0; i < length; i) {
        if (Number(daysDates[i].id) < dd && daysDates[i] != undefined) {
            daysDates[i].classList.add("daysGone");
            daysDates[i].classList.remove("days-dates");
            length--;
        } else {
            i++;
        }
    ...
    for (var b = 0; b < length; b++) {
        ...
        daysDates[b].addEventListener("click", function () {
            if (this.id.toString() != "secondDate" && firstDate == 0 && (secondDate > Number(this.id) || secondDate == 0)) {
                this.classList.add("firstDateCss");

                firstDate = this.id;
                this.id = "firstDate";
            } else if (this.id != "firstDate" && secondDate == 0 && (firstDate < this.id || firstDate == 0)) {
                this.classList.add("secondDateCss");
                ...
                this.classList.remove("firstDateCss");

                this.id = firstDate;
                firstDate = 0;
            } else if (this.id == "secondDate") {
                this.classList.remove("secondDateCss");
            ...
            // Blakk magikk
                ...
                let dateElement = document.getElementById("firstDate");
                dateElement.classList.remove("firstDateCss");
                dateElement.id = `${firstDate.toString()}`;
            ...
            // Blakk magikk
            else if (Number(this.id) > secondDate && secondDate != 0) {
                let dateElement = document.getElementById("secondDate");
                dateElement.classList.remove("secondDateCss");
                dateElement.id = `${secondDate.toString()}`;
